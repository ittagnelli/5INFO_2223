...
public class garabello_tcp_server {
    public static void main(String[] args) {
        ...
        try {
            ...
            while(true) {
                ...
                System.out.format("Java TCP server ha ricevuto dal client %d byte: %s\n", messaggio.length(), messaggio);

                OutputStream output = client_connection.getOutputStream();
                PrintWriter writer = new PrintWriter(output, true);
                writer.println(messaggio);

                System.out.format("Java TCP server ha inviato al client %d byte: %s\n", messaggio.length(), messaggio);
