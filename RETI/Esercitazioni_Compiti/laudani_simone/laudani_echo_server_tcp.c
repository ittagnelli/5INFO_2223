...
#include <arpa/inet.h>
//#include <winsock2.h> librerie per windows??
...
int main(int argc, char **argv) 
{
    ...
    if(argc != 2) { //diverso da 2 perche' l'argomento DEVE avere 2 valori: in linux per avviare la porta sono necessari 2 valori (./a.out num_porta)
        printf("uso: %s <porta>\n", argv[0]); // %s stampa "argv[0]"
        exit(1); //praticamente sto sgridando l'utente
    }
    ...
    for(;;) {
        ...
        //ESERCIZIO 3
        socket_send(connection_fd, buf); //incolliamo socket send dal server, rinvio al client quello che mi e' arrivato dal server
        //sostituito argv[] con buf
        //FINE
