...
#define BUFSIZE 1024   //ridichiarata la dimensione del buffer 
...
void socket_conenct(int socket_fd, char *ip, unsigned short tcp_port)
{
...
int socket_recive(int socket_fd, char *buf)   // Implemento il socket_recive() nel client per poter far tornare il messaggio.La funzione è cut and paste da server_tcp
{
    int msg_size;
...
int main(int argc, char **argv) 
{
    ...
    socket_conenct(socket_fd, ip, tcp_port);
    printf("Socket connesso con il server %s sulla porta %d\n", ip, tcp_port);

    /* invio sul socket la stringa */
     byte_sent = socket_send(socket_fd, argv[3]); 

    socket_recive(socket_fd, buf);  //chiamo la funzione passandogli il socketfd al posto del connection perchè client side ho questo.
    printf("Inviato %d bytes con successo\n", byte_sent);
    printf("Messaggio inviato: %s \n", buf );  //output del messaggio di ritorno
