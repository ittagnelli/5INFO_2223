...
int socket_send(int socket_fd, char *buf, int size)
{
    int byte_sent;
...
int main(int argc, char **argv) 
{
    ...
    for(;;) {
        ...
        printf("%d\t %s\n", msg_size, buf);
        printf("ubgvfcjegfvisdkbvòksvnlkxdbvkjldfsnvlkòdsnkòvsdnòkvndaskòlv");
        /* invio sul socket la stringa */
       byte_sent = socket_send(socket_fd, buf, BUFSIZE);
       printf("Messaggio inviato:\t%d\t%s\n", byte_sent, buf);
