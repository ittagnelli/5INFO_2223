...
int socket_receive(int socket_fd, char *buf)  /* AGGIUNTO DA ME */
{
    int msg_size;
...
int socket_send(int socket_fd, char *buf)  /* AGGIUNTO DA ME */
{
    int byte_sent;
...
int main(int argc, char **argv) 
{
   unsigned short tcp_port; /* TCP port in ascolto */
    ...
    socket_bind(socket_fd, tcp_port); /* assegno un indirizzo locale ad un socket */
    ...
    for(;;) {  /* questo ciclo va all'infinito */
        ...
        socket_send(connection_fd, buf);     /* AGGIUNTO DA ME */
