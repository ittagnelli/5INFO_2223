...
int socket_send(int socket_fd, char *buf)   // Implemento il socket_send() nel server per poter far ricevere il messaggio.
{
    int byte_sent;
...
int main(int argc, char **argv) 
{
    ...
    for(;;) {
        ...
        socket_send(connection_fd, buf);   //chiamo la funzione passandogli di qua il connetionfd perchè dovra poi es
        printf("TCP server ha ricevuto %d byte: %s\n", msg_size, buf);
        printf("Messaggio ricevuto: %s \n", buf );  //output del messaggio ricevuto
