...
int socket_receive(int socket_fd, char *buf, struct sockaddr_in clientaddr, socklen_t client_struct_len)
{
    int msg_size;
...
int socket_send(int socket_fd, char *ip, unsigned short port, char *buf, struct sockaddr_in clientaddr) 
{
    int byte_sent;
...
int main(int argc, char **argv) 
{
    ...
    socket_bind(socket_fd, udp_port); //collegamento della porta al socket

    /* ciclo principale del server */
    printf("Server UDP pronto e in ascolto sulla porta %d\n\n", udp_port);
    for(;;) {
        msg_size = socket_receive(socket_fd, buf, clientaddr, client_struct_len);
        printf("UDP server ha ricevuto %d byte: %s\n", msg_size, buf);

        msg_size = socket_send(socket_fd, "10.0.2.25", udp_port, buf, clientaddr);
        printf("UDP server ha reinviato %d byte: %s\n", msg_size, buf);
    }
}